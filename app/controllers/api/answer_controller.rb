  ...
  class AnswerController < ApplicationController
    def get
      ...
      rows = res.map {|r| r.id}.uniq.map {|id|
        ...
        {
          id: Answer.encode_id(data.first.id),
          question_id: Question.encode_id(data.first.question_id),
          content: data.first.content,
          good_cnt: data.first.good_cnt,
          bad_cnt: data.first.bad_cnt,
          is_best_answer: data.first.is_best_answer,
          user_id: User.encode_id(data.first.user_id),
          ...
          replies: data.first.reply_id.nil? ?
            [] : data.map {|d|
              {
                id: Reply.encode_id(d.reply_id),
                user_id: User.encode_id(d.reply_user_id),
